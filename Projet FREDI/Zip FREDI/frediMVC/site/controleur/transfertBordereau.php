...
verifSession();//voir controleurPrincipale.php
			...
			$prenom = $_SESSION['prenomD'];
 			$modeCo=$_SESSION['modeCO'];
 			 if($modeCo==0){
 				$nbrLignesFrais=getNbrLinesByNumDemandeurRL($_SESSION['numD']);
 				$nbrAD=getNbrOfAd($_SESSION['numD']);
 			}else{
 				$nbrLignesFrais=getNbrLinesByNumDemandeurAD($_SESSION['numD']);
 			}
			...
			if($_SESSION['transfert']==1 || $_SESSION['ouvert']==0){
				...
				include_once "$racine/vue/transfertBordereau.php";
			}

			if(isset($_POST['transferer'])){
				header("Location: ./?action=transfertValide");
